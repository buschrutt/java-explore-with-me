...
public interface StatsRepository extends JpaRepository<Hit, Integer> {
    @Query("SELECT new ru.practicum.explore.stats.model.Stats(s.app, s.uri, COUNT(s)) " +
    ...
    @Query("SELECT new ru.practicum.explore.stats.model.Stats(s.app, s.uri, COUNT(DISTINCT s.ip)) " +
            "FROM Hit s WHERE s.uri IN :uris AND s.viewDate >= :from AND s.viewDate <= :to " +
